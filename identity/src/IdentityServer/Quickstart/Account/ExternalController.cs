...
namespace BeansIdentityService.Server.Quickstart.Account;
...
public class ExternalController : Controller
{
    ...
    public async Task<IActionResult> Callback()
    {
        ...
        {
            IEnumerable<string> externalClaims = result.Principal.Claims.Select(
                c => $"{c.Type}: {c.Value}"
            );
        ...
        (IdentityUser user, string provider, string providerUserId,
                IEnumerable<Claim> claims) =
            await FindUserFromExternalProviderAsync(
                result
            );
        ...
        additionalLocalClaims.AddRange(
            principal.Claims
        );

        string name = principal.FindFirst(
                              JwtClaimTypes.Name
                          )
                          ?.Value
                      ?? user.Id;

        IdentityServerUser isuser = new IdentityServerUser(
            user.Id
        )
        {
            DisplayName = name,
    ...
    private async
        Task<(IdentityUser user, string provider, string providerUserId,
            IEnumerable<Claim> claims)> FindUserFromExternalProviderAsync(
        ...
        )
    {
        ...
        Claim userIdClaim = externalUser.FindFirst(
                                JwtClaimTypes.Subject
                            )
                            ?? externalUser.FindFirst(
    ...
    )
    {
        ...
        // user's display name
        string name = claims.FirstOrDefault(
        ...
        {
            string first = claims.FirstOrDefault(
                                   x => x.Type == JwtClaimTypes.GivenName
                               )
                               ?.Value
                           ?? claims.FirstOrDefault(
                                   x => x.Type == ClaimTypes.GivenName
                               )
                               ?.Value;

            string last = claims.FirstOrDefault(
                                  x => x.Type == JwtClaimTypes.FamilyName
                              )
                              ?.Value
                          ?? claims.FirstOrDefault(
                                  x => x.Type == ClaimTypes.Surname
                              )
                              ?.Value;
        ...
        // email
        string email = claims.FirstOrDefault(
                               x => x.Type == JwtClaimTypes.Email
                           )
                           ?.Value
                       ?? claims.FirstOrDefault(
    ...
    )
    {
        ...
        Claim sid = externalResult.Principal.Claims.FirstOrDefault(
        ...
        string idToken = externalResult.Properties.GetTokenValue(
            "id_token"
        );
