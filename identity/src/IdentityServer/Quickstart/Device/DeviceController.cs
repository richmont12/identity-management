...
public class DeviceController : Controller
{
    ...
    {
        string userCodeParamName = _options.Value.UserInteraction
        ...
        DeviceAuthorizationViewModel vm = await BuildViewModelAsync(
            userCode
        );
    ...
    )
    {
        ...
        ProcessConsentResult result = await ProcessConsent(
            model
        );
    ...
    )
    {
        ...
        if (grantedConsent != null)
        {
            ...
            await _interaction.HandleRequestAsync(
                model.UserCode,
                grantedConsent
            );
        ...
        else
        {
            ...
            result.ViewModel = await BuildViewModelAsync(
                model.UserCode,
                model
            );
        }
    ...
    )
    {
        ...
        vm.IdentityScopes = request.ValidatedResources.Resources
            .IdentityResources.Select(
        ...
        foreach (ParsedScopeValue parsedScope in request.ValidatedResources
                     .ParsedScopes)
        {
            ApiScope apiScope =
            ...
            if (apiScope != null)
            {
                ScopeViewModel scopeVm = CreateScopeViewModel(
                    parsedScope,
                    apiScope,
                    vm.ScopesConsented.Contains(
